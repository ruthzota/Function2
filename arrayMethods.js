...
const myStrongest = monstersInYourPost.filter(element => element.CP > 200)
...
const allOrders = orders.map(function(element){
  return element.price + (element.price * element.tax) 
})

const allOrders = order.map(element => element)
